    ...
    return (
        <div className="mb-12">
            <div className="flex items-center justify-between mb-4">
                <div>
                    <h2 className="text-xl font-bold text-gray-900 dark:text-white">Fresh Drops</h2>
                    <p className="text-sm text-gray-600 dark:text-white/60 mt-1">Check out brand-new sample packs newly added to the library.</p>
                </div>
            ...
            >
                {loading ? (
                    <div className="flex gap-4">
                        {[1, 2, 3, 4].map(i => (
                            <div key={i} className="flex-shrink-0 w-64 h-64 bg-gray-200 dark:bg-white/5 rounded-lg animate-pulse" />
                        ))}
                    </div>
                ) : newPacks.length === 0 ? (
                    <div className="text-gray-500 dark:text-white/40 text-center py-12 w-full">
                        No packs yet.
                    </div>
                ) : newPacks.map((pack) => {
                    return (
                        <motion.div
                            key={pack.id}
                            className="flex-shrink-0 w-64 group cursor-pointer p-3 rounded-2xl bg-gray-100/80 dark:bg-white/5 border border-gray-200 dark:border-white/10 backdrop-blur-sm hover:bg-gray-200/80 dark:hover:bg-white/10 hover:border-gray-300 dark:hover:border-white/20 transition-all"
                            whileHover={{ y: -4 }}
                            onClick={() => handlePackClick(pack.title)}
                        >
                            <div className={`w-full aspect-square rounded-xl mb-3 bg-gradient-to-br from-gray-200 dark:from-gray-900 to-gray-300 dark:to-gray-800 relative overflow-hidden border border-gray-200 dark:border-white/5 group-hover:border-green-500/50 transition-colors`}>
                                {/* Image */}
